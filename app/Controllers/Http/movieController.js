...
class movieController {
  async getMov({view, params}) {
    ...
    const film3 = await Database.table('movies').where({id: ((parseInt(params.id)+1))}).orWhere({id: ((parseInt(params.id)-1))}).first();
    const film4 = await Database.table('movies').where({id: ((parseInt(params.id)+2))}).orWhere({id: ((parseInt(params.id)-21))}).first();
